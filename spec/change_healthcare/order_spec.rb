...
RSpec.describe ChangeHealthcare::Order, :vcr do
  ...
  describe ".search_by_order_info" do
    it "Retrieves the next requisition number for the lab" do
      resp = ChangeHealthcare::Order.search_by_order_info({orderingorganization: facility })
  ...
  describe ".e_order_html_link" do
    let(:expected_response) do
      "https://cli-cert.changehealthcare.com/servlet/DxLogin?PW=#{password}&actionCommand=print&apiLogin=true&apiuserid=#{user_id}&hdnBusiness=#{facility}&orderid=3004834601&target=servlet%2Fservlets.apiOrderServlet%3F&userid=#{user_id}"
    end
